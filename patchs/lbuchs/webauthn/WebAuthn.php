...
use lbuchs\WebAuthn\Attestation\{
    AuthenticatorData, 
    AttestationObject
};
...
class WebAuthn {
    ...
    public function getChallenge() {
        return serialize($this->_challenge);
    }
    ...
    public function processCreate($clientDataJSON, $attestationObject, $challenge, $requireUserVerification=false, $requireUserPresent=true, $failIfRootMismatch=true) {
         ...
         */
        $challenge = $this->fixObject($challenge);
        $challenge = is_string($challenge) ? unserialize($challenge) : $challenge;
        // !! END PATCH !! //
